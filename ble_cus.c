...
// radio_mode_t previous_mode = BLE;
...
static void nus_data_handler(ble_nus_evt_t * p_evt)
{
    ...
    if (p_evt->type == BLE_NUS_EVT_RX_DATA)
    {
        ...
        // NRF_LOG_INFO("BLE: %s",p_evt->params.rx_data.p_data );
        // static char nus_message[32];
        // memcpy(nus_message, 0,sizeof(nus_message));
...
static void advertising_init(void)
{
    ...
    NRF_LOG_INFO("ble_advertising_init");
    NRF_LOG_FLUSH();
...
void ble_cus_init(void)
{
    ...
    NRF_LOG_INFO("Powermanagement Init");
    NRF_LOG_FLUSH();
